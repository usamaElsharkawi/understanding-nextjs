...
- **Session Cookies:** Deleted when the browser closes.
- **Persistent Cookies:** Have an `Expires` or `Max-Age` attribute.
- **Domain/Path Scope:** Cookies are sandboxed to specific domains and sub-paths for privacy and security.
</details>
...
- **Goal:** Understand user behavior and ad attribution.
- **Compliance:** Requires careful handling of user consent (GDPR/CCPA) as these are often "Persistent" cookies that stay on the device for months.
</details>
